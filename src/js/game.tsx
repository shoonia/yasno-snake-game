...
const food: IPoint = {
  x: randomInt(0, canvas.width / state.cellSize) * state.cellSize,
  y: randomInt(0, canvas.height / state.cellSize) * state.cellSize,
};
...
const ready = () => {
  const matches = window.innerWidth <= 650;

  canvas.width = matches ? 300 : 600;
  canvas.height = matches ? 300 : 480;
  ctx.fillRect(0, 0, matches ? 300 : 600, matches ? 300 : 480);
  state.cellSize = matches ? 18 : 24;
...
const checkBorder = () => {
  if (state.x < 0) {
    state.x = canvas.width - state.cellSize;
  } else if (state.x >= canvas.width) {
    state.x = 0;
  }
  if (state.y < 0) {
    state.y = canvas.height - state.cellSize;
  } else if (state.y >= canvas.height) {
    state.y = 0;
  }
};

const withoutBorder = () => {
  if (state.x < 0 || state.y < 0 || state.x >= canvas.width || state.y >= canvas.height) {
    audioPlay('end');
    restart();
  }
};
...
function drawSnake() {
  ...
  state.items.forEach((e, index) => {
    ctx.drawImage(img, e.x, e.y, state.cellSize, state.cellSize);
